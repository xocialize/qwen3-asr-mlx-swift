...
### Deliberate deviations from upstream (the emitted text only — never the model's input)

- **0.1.2 — a separator after a restart.** When the loop restarts from an empty prefix, the model
  begins a fresh transcript whose first token has no leading space. A restart comes after a pause
  longer than the 16 s window, whose drop leaves no committed text in the prompt, and after a
  hallucination reset. Upstream and the Python rung concatenate verbatim, so English after a 20 s
  pause read `the lakesOn August` and `she writesCome, come!` (ML[X] Audio Studio M14-C item 6).
  `R2T2Text.joining` puts a space in front of the emitted piece only on a restart, only between
  space-delimited scripts, and never before closing punctuation or around Han, kana or Thai.
  Mid-stream deltas are subword continuations and are never touched. `chunkText`, the prompt
  prefix, keeps the model's own text, so decoding is identical to upstream. G4 is still
  token-exact on every golden: none restarts after committed text.
- **0.1.1 — language hints.** A BCP-47 locale resolves by its primary subtag, and an unsupported
  language becomes nil (auto-detect) rather than an invented name in the prompt.

### Memory over a long session (0.1.3)

The window is bounded, so a session's memory should not grow with its length. Through 0.1.2 it
did. ML[X] Audio Studio's 45-minute live run (M14-C item 2) peaked 6.18 GB in its first five
minutes and 6.47 GB in its last, about +0.07 GB per 10 minutes of audio. `qwen3asr-gates creep` found where: MLX's
live arrays stayed flat (2.53 GB) and its pool sat at its cap, but the malloc heap grew about
6 MB a minute. Two heap snapshots 45 s apart named the growth: +45 k `NSConcreteValue`, +712 regex
match results, +89 autorelease-pool pages, and +517 Metal command-buffer completion handlers with
their buffers. Every step hands back AUTORELEASED objects: the tokenizer's `NSRegularExpression`
pre-tokenizer, run on each step's prompt tail, and MLX's Metal command buffers. The host's loop,
whether a Swift-concurrency job or a CLI's `run()`, never drained a pool. **`push` and `finish` now
run each step inside `autoreleasepool`**, and the heap holds flat (0.221 → 0.220 → 0.221 → 0.221 GB
over minutes 1–4, against 0.228 → 0.246 before). Text is unchanged, and G4 is token-exact on all five goldens.
